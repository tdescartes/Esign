...
documents     id, owner_id, title, pages, status(draft|sent|completed), page_count,
              source_file_path, sealed_file_path, sealed_hash, created_at, completed_at
...
in the right place regardless of the device rendering it. Documents are owned by a
single authenticated sender (`owner_id`); every sender-side endpoint requires a
session and is scoped to that owner.

Persistence is Postgres via `DATABASE_URL`. When unset, the API falls back to an
in-memory Postgres-compatible engine (`pg-mem`) for zero-config local dev and
tests only — never rely on that fallback in production.

## API surface

Swagger UI is served at `/api/docs/`, with the generated OpenAPI document at
`/api/docs.json`.

| Method & path                           | Who    | Purpose                                                     |
| --------------------------------------- | ------ | ----------------------------------------------------------- |
| `POST /api/auth/signup`                 | anyone | Create a sender account; sets a session cookie               |
| `POST /api/auth/login`                  | anyone | Start a session                                              |
| `POST /api/auth/logout`                 | sender | Revoke the current session                                   |
| `GET  /api/me`                          | sender | Get the authenticated sender                                 |
...
| `POST /api/documents/:id/file`          | sender | Upload the real source PDF (validated, page count extracted) |
| `GET  /api/documents/:id/file`          | sender | Download the uploaded source PDF                              |
| `GET  /api/documents/:id/sealed-file`   | sender | Download the flattened, sealed PDF                            |
...
| `GET  /api/sign/:token/file`            | signer | Stream the source PDF for viewing                             |
...
| ------------- | ------------------------------ | --------------------------------- | --------------------------------------------------------------------------- |
| Document      | HTML "pages"                  | HTML pages, or a real uploaded PDF | Real PDF via **pdf.js** (render in the editor) + **pdf-lib** (flatten)      |
...
| Integrity     | Web Crypto SHA-256            | SHA-256 of the flattened PDF when one was uploaded; canonical-JSON hash otherwise | trusted timestamp (RFC 3161) added on top |
| Attribution   | fake IPs                      | request IP + user agent captured; consent recorded separately | + email/SMS OTP for higher assurance |
| Sender auth   | none                          | email/password + session cookie (bcrypt, httpOnly) | add email verification, MFA, password reset |
| Storage       | none                          | Postgres (pg-mem fallback for dev); PDFs on local disk | Postgres + S3-compatible object storage for PDFs |
