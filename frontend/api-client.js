// api-client.js — thin wrappers around the Esign API.
//
...
export const createDocument = (title, pages, recipients = [], fields = []) =>
  j(`${API}/documents`, {
    method: "POST",
    body: JSON.stringify({ title, pages, recipients, fields }),
  });

export const addRecipient = (docId, name, email) =>
  j(`${API}/documents/${docId}/recipients`, {
    method: "POST",
    body: JSON.stringify({ name, email }),
  });

export const addField = (docId, recipientId, field) =>
  j(`${API}/documents/${docId}/fields`, {
    method: "POST",
    body: JSON.stringify({ recipientId, ...field }),
  });
...
export const getCertificate = (docId) =>
  j(`${API}/documents/${docId}/certificate`);
...
export const fillField = (token, fieldId, value, valueMeta) =>
  j(`${API}/sign/${token}/fields/${fieldId}`, {
    method: "POST",
    body: JSON.stringify({ value, valueMeta }),
  });

export const finishSigning = (token, consent = true) =>
  j(`${API}/sign/${token}/complete`, {
    method: "POST",
