...
const COLORS = ["#0f9d6b","#2563eb","#c2410c","#7c3aed","#0891b2","#be185d","#4d7c0f","#b45309"];
...
function audit(documentId, actor, event, meta, ip) {
  db.prepare(`INSERT INTO audit_events (id,document_id,actor,event,meta,ip,created_at)
              VALUES (?,?,?,?,?,?,?)`)
    .run(randomUUID(), documentId, actor, event, meta || null, ip || null, now());
}
...
const getByTok = (t) => db.prepare("SELECT * FROM recipients WHERE token=?").get(t);
const recipientsOf = (id) => db.prepare("SELECT * FROM recipients WHERE document_id=? ORDER BY signing_order, created_at").all(id);
const fieldsOf = (id) => db.prepare("SELECT * FROM fields WHERE document_id=?").all(id);
...
app.post("/api/documents", (req, res) => {
  const { title = "Untitled", pages = [], recipients = [], fields = [] } = req.body;
  const id = randomUUID();
  db.prepare(`INSERT INTO documents (id,title,pages,status,created_at) VALUES (?,?,?,?,?)`)
  ...
  recipients.forEach((r, i) => {
    ...
    recIdByIndex[i] = rid;
    db.prepare(`INSERT INTO recipients (id,document_id,name,email,color,signing_order,status,created_at)
                VALUES (?,?,?,?,?,?,?,?)`)
      .run(rid, id, r.name || `Signer ${i+1}`, r.email || null,
           r.color || COLORS[i % COLORS.length], r.signingOrder || 0, "draft", now());
  });
  fields.forEach(f => insertField(id, f.recipientId ?? recIdByIndex[f.recipientIndex], f));
...
app.post("/api/documents/:id/recipients", (req, res) => {
  ...
  db.prepare(`INSERT INTO recipients (id,document_id,name,email,color,signing_order,status,created_at)
              VALUES (?,?,?,?,?,?,?,?)`)
    .run(rid, doc.id, name || `Signer ${count+1}`, email || null,
         COLORS[count % COLORS.length], signingOrder, "draft", now());
  res.status(201).json(db.prepare("SELECT * FROM recipients WHERE id=?").get(rid));
});
...
app.post("/api/documents/:id/fields", (req, res) => {
  ...
  if (!field) return res.status(400).json({ error: "invalid recipientId or field" });
...
function insertField(documentId, recipientId, f) {
  ...
  const id = randomUUID();
  db.prepare(`INSERT INTO fields (id,document_id,recipient_id,type,page,x_pct,y_pct,w,h,required,created_at)
              VALUES (?,?,?,?,?,?,?,?,?,?,?)`)
    .run(id, documentId, recipientId, f.type, f.page ?? 0,
         f.xPct ?? f.x_pct ?? 0, f.yPct ?? f.y_pct ?? 0, f.w ?? 150, f.h ?? 40,
         f.required === false ? 0 : 1, now());
...
app.post("/api/documents/:id/send", (req, res) => {
  ...
  if (!recs.length) return res.status(400).json({ error: "add at least one recipient" });
  if (!fieldsOf(doc.id).length) return res.status(400).json({ error: "add at least one field" });

  db.prepare("UPDATE documents SET status='sent' WHERE id=?").run(doc.id);
  const links = recs.map(r => {
    const token = randomBytes(24).toString("hex");
    db.prepare("UPDATE recipients SET token=?, status='sent' WHERE id=?").run(token, r.id);
...
app.get("/api/documents/:id/audit", (req, res) => {
  const doc = getDoc(req.params.id);
  if (!doc) return res.status(404).json({ error: "document not found" });
  res.json(db.prepare("SELECT actor,event,meta,ip,created_at FROM audit_events WHERE document_id=? ORDER BY created_at").all(doc.id));
});

app.get("/api/documents/:id/certificate", (req, res) => {
  const doc = getDoc(req.params.id);
  if (!doc) return res.status(404).json({ error: "document not found" });
  if (doc.status !== "completed") return res.status(409).json({ error: "not completed yet" });
  res.json({
    document: doc.title,
    envelopeId: doc.id,
    sealedHash: doc.sealed_hash,
    completedAt: doc.completed_at,
    signers: recipientsOf(doc.id).map(r => ({ name: r.name, email: r.email, status: r.status })),
    audit: db.prepare("SELECT actor,event,meta,ip,created_at FROM audit_events WHERE document_id=? ORDER BY created_at").all(doc.id),
  });
});
...
app.get("/api/sign/:token", (req, res) => {
  ...
  res.json({
    document: { title: doc.title, pages: JSON.parse(doc.pages), status: doc.status },
    me: { id: rec.id, name: rec.name, email: rec.email, color: rec.color, status: rec.status },
    // return all fields but mark which are the signer's; others render as locked context
    fields: all.map(f => ({
      id: f.id, type: f.type, page: f.page, xPct: f.x_pct, yPct: f.y_pct, w: f.w, h: f.h,
      required: !!f.required, value: f.value, mine: f.recipient_id === rec.id,
...
app.post("/api/sign/:token/fields/:fieldId", (req, res) => {
  ...
  const field = db.prepare("SELECT * FROM fields WHERE id=? AND document_id=?").get(req.params.fieldId, rec.document_id);
  if (!field || field.recipient_id !== rec.id) return res.status(403).json({ error: "not your field" });
  db.prepare("UPDATE fields SET value=?, value_meta=? WHERE id=?")
    .run(req.body.value ?? null, req.body.valueMeta ? JSON.stringify(req.body.valueMeta) : null, field.id);
  res.json({ ok: true });
});
...
app.post("/api/sign/:token/complete", (req, res) => {
  ...
  const mine = db.prepare("SELECT * FROM fields WHERE recipient_id=?").all(rec.id);
  const missing = mine.filter(f => f.required && (f.value === null || f.value === ""));
  if (missing.length) return res.status(400).json({ error: `${missing.length} required field(s) unfilled` });

  const ip = ipOf(req);
  db.prepare("UPDATE recipients SET status='signed' WHERE id=?").run(rec.id);
  audit(rec.document_id, rec.name, "consent", "Accepted electronic signature consent (ESIGN/UETA)", ip);
  audit(rec.document_id, rec.name, "signed", rec.email ? `<${rec.email}>` : null, ip);
  emit("recipient.signed", { documentId: rec.document_id, recipient: rec.name });

  // all signed? seal it.
  const recs = recipientsOf(rec.document_id);
  if (recs.every(r => r.status === "signed")) {
    const doc = getDoc(rec.document_id);
    const { hash } = sealDocument({ document: doc, recipients: recs, fields: fieldsOf(doc.id) });
    db.prepare("UPDATE documents SET status='completed', sealed_hash=?, completed_at=? WHERE id=?")
      .run(hash, now(), doc.id);
...
function recipientColor(docId, recipientId) {
  const r = db.prepare("SELECT color FROM recipients WHERE id=?").get(recipientId);
...
function fullDocument(id) {
  ...
  return {
    ...
    pages: JSON.parse(doc.pages), sealedHash: doc.sealed_hash,
    createdAt: doc.created_at, completedAt: doc.completed_at,
    recipients: recipientsOf(id).map(r => ({
      id: r.id, name: r.name, email: r.email, color: r.color,
      status: r.status, signingOrder: r.signing_order,
    ...
    fields: fieldsOf(id).map(f => ({
      ...
      xPct: f.x_pct, yPct: f.y_pct, w: f.w, h: f.h, required: !!f.required, value: f.value,
    })),
  };
}
